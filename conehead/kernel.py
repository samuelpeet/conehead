...
class KernelMono:
    ...
    def _from_egslst(self, egslst_path: str):
        with open(egslst_path) as f:
            ...
            n_cones = 12
            n_depths = 36
            self.kernel_raw = data_raw[:, 2].reshape((n_cones, n_depths))
            self.kernel = data_raw[:, 2].reshape((n_cones, n_depths))
            ...
            # volumes = self._spherical_voxel_volumes(r_edges, angles_edges)
            # self.kernel = self.kernel / volumes
            # self.kernel = self.kernel / self.kernel.sum()
            ...
            for i in range(self.kernel.shape[0]):
                ...
                # dTheta = np.cos(angles_edges[i]) - np.cos(angles_edges[i+1])
                # self.kernel[i, :] = self.kernel[i, :] / (2 * np.pi * dTheta * dR)
                w = 2 * np.pi * (np.cos(angles_edges[i]) - np.cos(angles_edges[i+1]))  # Angular weight for azimuthal symmetry
                self.kernel[i, :] = self.kernel[i, :] * w  # Convert to per unit solid angle
           
            #     self.kernel[i, :] = self.kernel[i, :] * self.radii_centres**2  # Convert to * cm^2
