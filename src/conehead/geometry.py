...
def line_block_plane_collision(ray_start: npt.NDArray[np.float32], ray_direction: npt.NDArray[np.float32], plane_normal: npt.NDArray[np.float32], epsilon: np.float32 = 1e-6) -> npt.NDArray[np.float32]:
    """ Calculate the point of intersection of a line and the blocking plane.

     Parameters
    ----------
    ray_start : ndarray
        Position of point on line, typically the source position will be used
    ...
    plane_point: npt.NDArray[np.float32] = np.array([0, 0, 0], dtype=np.float32)  # Isocentre
    # ray_point = np.array([0, 0, 0])  # Source position

    ndotu = plane_normal.dot(ray_direction)
    if abs(ndotu) < epsilon:
        raise RuntimeError("no intersection or line is within plane")
    w = ray_start - plane_point
